...
exports.registro = async (req, res)=>{ 
   try { 
      const user = req.body.user; //solicitamos los datos del formulario
      const correo = req.body.correo; 
      const pass = req.body.pass; 
      let passHash = await bcryptjs.hash(pass, 8) //hasheamos la contraseña
      console.log(passHash) 
      //consultamos a la bd por los datos 
...
exports.login = async (req, res)=>{ 
   try { 
      ...
      if(!user || !pass){ 
         ...
         //si no estan vacios consultamos en la bd si los datos son validos. 
...
exports.isAuthenticated = async (req, res, next)=>{ 
   ...
   if(req.cookies.jwt){ 
      try { 
         ...
         //query para consultar en la base de datos si existe el usuario ingresado
      ...
      //si las credenciales no son validos, los enviaremos a la pagina de login  
      res.redirect('/login') 
   } 
} 
